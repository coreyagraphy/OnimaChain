...
function settings() {
  const operator = process.env.EDITORIAL_OPERATOR_NAME?.trim() ?? ''
  const inbox = process.env.EDITORIAL_CONTACT_EMAIL?.trim() ?? ''
...
export default async (request: Request) => {
  ...
  if (request.method === 'GET') return json(config.ready ? { ready: true, operator: config.operator, email: config.inbox } : { ready: false })
