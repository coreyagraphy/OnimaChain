...
import { rootRouteId, invariant, isNotFound, isRedirect, getScriptPreloadAttrs, getStylesheetHref, resolveManifestCssLink, resolveManifestAssetLink, createSerializationAdapter, isPromise, _getRenderedMatches, executeRewriteInput, isDangerousProtocol } from "@tanstack/router-core";
import { makeSerovalPlugin, mergeHeaders } from "@tanstack/router-core/ssr/client";
import { createRawStreamRPCPlugin, defaultSerovalDeserializerPlugins, getNormalizedURL, waitForRequest, normalizeSsrResponse, attachRouterServerSsrUtils, stripSsrResponseBody, bindSsrResponseToRequest, replaceSsrResponse, isSsrResponse, disposeSsrResponse } from "@tanstack/router-core/ssr/server";
...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("./assets/_tanstack-start-manifest_v-C_T8ZbrA.js");
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import("./assets/router-CxtrX1wR.js").then((n) => n.aF),
