...
for (const [width, height] of [[390, 900], [1440, 900]]) {
  ...
  if (width === 1440) {
    ...
    await secondRow.getByRole('combobox', { name: 'Epitalon strength' }).selectOption('example-10mg')
    ...
    await cart.waitFor()
    assert.match(await cart.innerText(), /Example 10 mg · \$YY\.YY/)
    await cart.getByRole('button', { name: 'Close cart' }).last().click()
    await secondRow.getByRole('combobox', { name: 'Epitalon strength' }).selectOption('example-5mg')
    await secondRow.getByRole('button', { name: 'Add to cart' }).click()
    assert.equal(await cart.locator('.cart-line').count(), 2, 'Different strengths remain separate cart lines')
    await cart.locator('.cart-line').filter({ hasText: 'Example 10 mg' }).getByRole('button', { name: 'Remove' }).click()
    assert.equal(await cart.locator('.cart-line').count(), 1, 'Removing one strength retains the other')
    assert.match(await cart.innerText(), /Example 5 mg · \$XX\.XX/)
    await cart.getByRole('button', { name: 'Close cart' }).last().click()
    await page.getByRole('button', { name: 'List', exact: true }).click()
    const epitalonRow = page.locator('.commerce-table tr').filter({ has: page.getByRole('link', { name: 'Epitalon', exact: true }) })
    await epitalonRow.getByRole('combobox', { name: 'Epitalon strength' }).selectOption('example-10mg')
