...
for (const [width, height] of [[390, 900], [1440, 900]]) {
  ...
  if (width === 1440) {
    const secondRow = page.locator('.product-card').filter({ has: page.getByRole('heading', { name: 'Epitalon', exact: true }) })
    await secondRow.scrollIntoViewIfNeeded()
    await page.waitForTimeout(1200)
    await page.screenshot({ path: 'shots/portal-refresh/shop-second-row-1440.png' })
    await secondRow.screenshot({ path: 'shots/portal-refresh/epitalon-contained-1440.png' })
    assert.equal(await secondRow.locator('.product-molecule canvas').count(), 1, 'A visible card keeps its 3D canvas inside the artwork area')
    await secondRow.getByRole('combobox', { name: 'Epitalon dosage' }).selectOption('dosage-b')
    assert.match(await secondRow.locator('.strength-price-value').innerText(), /\$YY\.YY/)
    await secondRow.getByRole('button', { name: 'Add to cart' }).click()
    const cart = page.getByRole('dialog', { name: 'OnimaChain shopping cart' })
    await cart.waitFor()
    assert.match(await cart.innerText(), /Dosage B · TBD · \$YY\.YY/)
    await cart.getByRole('button', { name: 'Close cart' }).last().click()
    await secondRow.getByRole('combobox', { name: 'Epitalon dosage' }).selectOption('dosage-a')
    await secondRow.getByRole('button', { name: 'Add to cart' }).click()
    assert.equal(await cart.locator('.cart-line').count(), 2, 'Different dosage placeholders remain separate cart lines')
    await cart.locator('.cart-line').filter({ hasText: 'Dosage B · TBD' }).getByRole('button', { name: 'Remove' }).click()
    assert.equal(await cart.locator('.cart-line').count(), 1, 'Removing one dosage placeholder retains the other')
    assert.match(await cart.innerText(), /Dosage A · TBD · \$XX\.XX/)
    await cart.getByRole('button', { name: 'Close cart' }).last().click()
    await page.getByRole('button', { name: 'List', exact: true }).click()
    const epitalonRow = page.locator('.commerce-table tr').filter({ has: page.getByRole('link', { name: 'Epitalon', exact: true }) })
    await epitalonRow.getByRole('combobox', { name: 'Epitalon dosage' }).selectOption('dosage-b')
    assert.match(await epitalonRow.innerText(), /\$YY\.YY/)
