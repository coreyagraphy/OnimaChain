...
export function ResearchEntityCard({ compound: c }: { compound: ResearchCompound }) {
  ...
  return <article id={c.id} className="research-entity-card" data-motion={theme.motionSignature} data-material={theme.moleculeMaterial} style={{ '--entity-accent': theme.primary, '--entity-secondary': theme.secondary } as React.CSSProperties}>
    <div className="research-entity-visual" aria-label={`Conceptual target visualization for ${c.name}; not a measured molecular structure`}>
    ...
    <div className="research-entity-body">
      ...
      <dl className="research-entity-facts mt-5"><div><dt>Development</dt><dd>{c.stage ?? 'Not assessed'}</dd></div><div><dt>Developer</dt><dd>{c.developer ?? 'Not mapped'}</dd></div><div><dt>Source date</dt><dd>{c.statusAsOf ?? 'Pending'}</dd></div></dl>
