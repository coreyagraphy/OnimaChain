...
import { createScrub, heroStoryMap } from '~/motion/timeline'

const HeroScene = lazy(() => import('~/scenes/hero/HeroScene').then((m) => ({ default: m.HeroScene })))
const CAPTIONS = [
  { from: 0.02, to: 0.2, k: 'Meet the molecule', v: 'BPC-157 is a chain of 15 amino acids.' },
  { from: 0.24, to: 0.58, k: 'Move closer', v: 'Travel between the glowing parts that make up the chain.' },
  { from: 0.6, to: 0.78, k: 'Read the story', v: 'See what was studied, what people say, and what is still unknown.' },
]
...
export function Hero() {
  ...
  useEffect(() => {
    ...
    const cleanup = createScrub({
      ...
      onProgress: (p) => {
        progress.current = p
        const story = heroStoryMap(p)
        if (copyRef.current) {
          const k = Math.max(0, Math.min(1, (p - 0.78) / 0.16))
          copyRef.current.style.opacity = String(k)
          copyRef.current.style.transform = `translateY(${(1 - k) * 42}px) scale(${0.96 + k * 0.04})`
          copyRef.current.style.pointerEvents = k < 0.8 ? 'none' : 'auto'
        }
        if (copyPlateRef.current) copyPlateRef.current.style.opacity = String(Math.max(0, Math.min(0.82, (p - 0.76) / 0.16)))
        if (barRef.current) barRef.current.style.transform = `scaleX(${story})`
        CAPTIONS.forEach((c, i) => {
          const el = capRefs.current[i]
          if (!el) return
          const o = Math.max(0, Math.min(1, (p - c.from) / 0.05, (c.to - p) / 0.05))
  ...
  return (
    <section ref={section} className="hero-shell relative w-full" aria-label="Molecular collection introduction">
      <div className="relative h-dvh w-full overflow-hidden bg-obsidian grain">
        <picture>
          <source media="(max-aspect-ratio: 4/5)" srcSet="/posters/hero-portrait.jpg" />
          <img
            src="/posters/hero.jpg"
            alt="A glowing sequence-derived molecular chain in a deep cyan and violet laboratory environment"
            width={1440}
            height={900}
            fetchPriority="high"
            decoding="async"
        ...
        {allowed && (
          ...
          >
            <Suspense fallback={null}>
              <HeroScene progress={progress} pointer={pointer} onRevealed={() => setRevealed(true)} />
            </Suspense>
          </Lod0Canvas>
        )}
        ...
        <div ref={copyPlateRef} className="hero-copy-plate" style={{ opacity: isStatic ? 0.82 : 0 }} aria-hidden />
        <div ref={copyRef} className="hero-copy-late relative z-10 h-full wrap flex flex-col justify-center" style={{ opacity: isStatic ? 1 : 0 }}>
          ...
          <div className="mt-9 flex flex-wrap gap-3">
            <Link to="/explore" className="btn btn-primary">Shop the collection</Link>
            <Link to="/signal" className="btn">Find a peptide</Link>
          </div>
        </div>
