...
export function LessonVisual({ kind }: { kind: VisualKind }) {
  return <div className={`lab-visual lab-visual--${kind}`} aria-hidden="true">
    ...
    {kind === 'dish' && <div className="lab-dish"><div className="lab-dish-rim">{Array.from({ length: 13 }, (_, i) => <i key={i} style={{ '--n': i } as CSSProperties} />)}</div><span>IN VITRO</span></div>}
    {kind === 'replication' && <div className="lab-replication"><span className="lab-replication-core">LAB 01</span><div>{Array.from({ length: 6 }, (_, i) => <i key={i}>P{i + 1}</i>)}</div><span className="lab-replication-other">LAB 02 ?</span></div>}
    ...
    {kind === 'paper' && <div className="lab-paper"><span>RESEARCH PAPER</span>{['Abstract', 'Methods', 'Results', 'Limits'].map((s) => <i key={s}>{s}</i>)}</div>}
