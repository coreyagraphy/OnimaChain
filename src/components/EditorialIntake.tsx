...
export function EditorialIntake({ page = '' }: { page?: string }) {
  ...
  useEffect(() => { let active = true; fetch('/api/editorial').then(response => response.ok ? response.json() as Promise<Channel> : { ready: false }).then(value => { if (active) setChannel(value) }).catch(() => { if (active) setChannel({ ready: false }) }); return () => { active = false } }, [])
  ...
  return <section className="panel glass relative p-6 md:p-8 max-w-3xl" aria-labelledby="editorial-intake-title">
    ...
    {channel?.ready ? <><p className="relative text-sm muted mt-3">Operator: {channel.operator} · Editorial inbox: <a href={`mailto:${channel.email}`} className="underline">{channel.email}</a></p><form onSubmit={submit} className="relative grid gap-4 mt-7">
    ...
    </form></> : <p className="relative text-sm text-bone/70 mt-6" role="status">Editorial intake is pending release. An operator name, controlled inbox, sender and successful delivery test are required before this form opens.</p>}
  </section>
}
