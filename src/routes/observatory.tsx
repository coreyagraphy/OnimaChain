...
const stations = [
  ...
  { id: 'report', number: '03', title: 'Report Detective', short: 'Report', subtitle: 'Read identity, purity, and concentration fields without overstating them.' },
...
const cases = [
  ...
  { tag: 'WHAT CHANGED?', title: 'Two ingredients. One changed outcome.', source: 'In a fictional teaching experiment, Group A receives an inactive comparison. Group B receives compounds X and Y together. No X-only or Y-only groups are included.', question: 'What can this design attribute to either ingredient?', choices: [
    { text: 'X caused the entire difference.', correct: false, why: 'There is no X-only group, so this design cannot isolate X.' },
    { text: 'The combined group differed; the contribution of X or Y remains unresolved.', correct: true, why: 'The group comparison may describe a difference, but it cannot assign it to a particular ingredient.' },
...
function ScaleLab() {
  ...
  return <div className="obs-simple-layout"><div className="obs-simple-intro"><span className="obs-kicker">SCALE LAB / ORDER THE LEVELS</span><h3>From nanometres<br/>to metres.</h3><p>Move these illustrative objects from smaller to larger. The labels teach relative size and units, not an exact measurement of a particular peptide or person.</p><div className="obs-insight">A cell observation and a human outcome also sit at different levels of evidence. Physical size alone does not bridge that gap.</div></div><div className="obs-sort-card"><div className="obs-sort-heading">DRAG-FREE / KEYBOARD FRIENDLY <span>USE THE ARROWS</span></div>{order.map((value, index) => { const item = scaleOrder[value]; return <div className="obs-sort-row" key={item.id}><span>{String(index + 1).padStart(2, '0')}</span><div><strong>{item.title}</strong><small>{item.detail}</small></div><div className="obs-sort-buttons"><button onClick={() => move(index, -1)} disabled={index === 0} aria-label={`Move ${item.title} up`}>↑</button><button onClick={() => move(index, 1)} disabled={index === order.length - 1} aria-label={`Move ${item.title} down`}>↓</button></div></div> })}<button className="obs-primary" onClick={() => setChecked(true)}>Check the order →</button><div className={`obs-result ${checked && correct ? 'right' : ''}`} aria-live="polite">{checked ? correct ? 'Exactly. Nanometres, micrometres, millimetres, then metres; each step is one thousand times the unit before it.' : 'Not quite. Start with the molecular feature, then move toward the whole person.' : 'Build the sequence to explore relative scale.'}</div></div></div>
}

const annotations = [
  { key: 'identity', label: 'Identity', value: 'Expected mass peak: present', detail: 'An identity test asks whether the sample matches the analyte the method was built to detect. It does not establish biological effect or human-use suitability.' },
  { key: 'purity', label: 'Purity', value: 'Main chromatographic peak: 96%', detail: 'A peak-area percentage depends on the analytical method. It is not a complete inventory of impurities or a safety score.' },
  { key: 'concentration', label: 'Concentration', value: 'Measured: 0.8 mg/mL', detail: 'Concentration describes amount per volume in this fictional sample. It is not an administration instruction and says nothing about clinical suitability.' },
] as const
