...
function Dossier() {
  ...
  return (
    <article className="pt-[72px] compound-world" style={{ '--product': productTheme.primary, '--product-2': productTheme.secondary, '--product-3': productTheme.tertiary } as CSSProperties}>
      ...
      {/* L — sources */}
      <Section id="sources" k="L" title="Sources" lede="Fully traceable bibliography. Only build-time-verified PMIDs are rendered as citations. FDA labels, when indexed, sit beside them — they are approvals, not shopping claims.">
        {official.length ? (
          ...
          <EmptyState title="No official labeling indexed for this compound" detail="FDA application records are attached only when the molecule is an approved drug and the application number has been checked. Most research peptides will stay empty here." />
        )}
        {studies.length ? (
          <div className="grid md:grid-cols-2 gap-3">{studies.map((s) => <StudyCard key={s.pmid} study={s} />)}</div>
        ) : official.length ? (
          <p className="text-sm muted mt-2">No PubMed records are indexed for this compound yet. The official labels above are the current source set.</p>
        ) : (
